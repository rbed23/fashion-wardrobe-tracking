...
def setup_logger(app, level=10):
    ...
    LOGGING_CONFIG = {
        ...
        'filters': {
            'contextual': {
                '()': 'fashion_wardrobe_tracker.logger.ContextualFilter'
            }
        },
        'formatters': { 
            'standard': { 
            ...
            'debug_format': {
                'format':   "%(asctime)s\t%(levelname)s\tUser <%(user_id)-3s> "
                            "from %(ip)-10s\t%(method)-5s\t%(url)s\t%(message)s\n"
